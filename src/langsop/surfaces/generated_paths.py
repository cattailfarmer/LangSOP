"""Generated path policy for IR5 surface projection outputs."""
...
NON_AUTHORITY_WARNING = (
    "surface projection output is generated-projection-only and does not replace signed SOP authority"
)

FIXTURE_SOURCE_ROOT = Path("tests/fixtures/sop/ir5_surface_projection")
EXPECTED_LEDGER_ROOT = Path("tests/fixtures/expected/ir5_surface_projection")
ACCEPTED_GENERATED_ROOTS: tuple[Path, ...] = (
    Path(".langsop/projections/ir5_surface_projection"),
    Path(".langsop/reports/ir5_surface_projection"),
    Path("tests/fixtures/generated/ir5_surface_projection"),
    Path("tests/fixtures/generated/ir5_surface_projection/projections"),
    Path("tests/fixtures/generated/ir5_surface_projection/reports"),
)
...
class SurfacePathPolicyResult:
    """Result of validating an IR5 surface projection path."""
...
def validate_tracked_surface_fixture_source_path(
    path: str | Path,
    *,
    workspace_root: str | Path = ".",
) -> SurfacePathPolicyResult:
    """Accept only tracked IR5 fixture source files under the fixture root."""

    return _validate_read_path(
        path,
        allowed_root=FIXTURE_SOURCE_ROOT,
        refusal_reason="surface_fixture_source_path_outside_policy",
        workspace_root=workspace_root,
    )


def validate_surface_expected_ledger_path(
    path: str | Path,
    *,
    workspace_root: str | Path = ".",
) -> SurfacePathPolicyResult:
    """Accept only tracked IR5 expected ledger files under the expected root."""

    return _validate_read_path(
        path,
        allowed_root=EXPECTED_LEDGER_ROOT,
        refusal_reason="surface_expected_ledger_path_outside_policy",
        workspace_root=workspace_root,
    )


def validate_generated_surface_output_path(
    path: str | Path,
    *,
    workspace_root: str | Path = ".",
    accepted_roots: Sequence[Path] = ACCEPTED_GENERATED_ROOTS,
) -> SurfacePathPolicyResult:
    """Accept generated surface output paths only under accepted roots."""
    ...
    if forbidden is not None:
        return SurfacePathPolicyResult(
    ...
    return SurfacePathPolicyResult(
        ...
        refusal_reason=None if matched is not None else "generated_surface_path_escape",
    )


def write_generated_surface_output(
    path: str | Path,
    content: Mapping[str, object] | object | str,
    *,
    workspace_root: str | Path = ".",
) -> SurfacePathPolicyResult:
    """Write generated surface output only after path-policy acceptance."""

    policy = validate_generated_surface_output_path(path, workspace_root=workspace_root)
