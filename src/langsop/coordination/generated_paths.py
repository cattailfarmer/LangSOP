"""Generated path policy for IR6 coordination projection outputs."""
...
NON_AUTHORITY_WARNING = (
    "coordination projection output is generated-projection-only and does not replace signed SOP authority"
)

FIXTURE_SOURCE_ROOT = Path("tests/fixtures/sop/ir6_coordination_mailbox")
EXPECTED_LEDGER_ROOT = Path("tests/fixtures/expected/ir6_coordination_mailbox")
PLATFORM_MAILBOX_ROOT = Path("platform/coordination/mailboxes")
ACCEPTED_GENERATED_ROOTS: tuple[Path, ...] = (
    Path(".langsop/projections/ir6_coordination_mailbox"),
    Path(".langsop/reports/ir6_coordination_mailbox"),
    Path("tests/fixtures/generated/ir6_coordination_mailbox"),
    Path("tests/fixtures/generated/ir6_coordination_mailbox/projections"),
    Path("tests/fixtures/generated/ir6_coordination_mailbox/reports"),
)
...
class CoordinationPathPolicyResult:
    """Result of validating an IR6 coordination path."""
...
def validate_tracked_coordination_fixture_source_path(
    path: str | Path,
    *,
    workspace_root: str | Path = ".",
) -> CoordinationPathPolicyResult:
    """Accept only tracked IR6 fixture source files under the fixture root."""

    return _validate_read_path(
        path,
        allowed_root=FIXTURE_SOURCE_ROOT,
        refusal_reason="coordination_fixture_source_path_outside_policy",
        workspace_root=workspace_root,
    )


def validate_coordination_expected_ledger_path(
    path: str | Path,
    *,
    workspace_root: str | Path = ".",
) -> CoordinationPathPolicyResult:
    """Accept only tracked IR6 expected ledger files under the expected root."""

    return _validate_read_path(
        path,
        allowed_root=EXPECTED_LEDGER_ROOT,
        refusal_reason="coordination_expected_ledger_path_outside_policy",
        workspace_root=workspace_root,
    )


def validate_generated_coordination_output_path(
    path: str | Path,
    *,
    workspace_root: str | Path = ".",
    accepted_roots: Sequence[Path] = ACCEPTED_GENERATED_ROOTS,
) -> CoordinationPathPolicyResult:
    """Accept generated coordination output paths only under accepted roots."""
    ...
    if forbidden is not None:
        return CoordinationPathPolicyResult(
            requested_path=str(path),
            accepted=False,
            normalized_path=str(candidate),
            matched_root=str(forbidden),
            refusal_reason="source_or_mailbox_mutation_refusal",
        )
    ...
    return CoordinationPathPolicyResult(
        ...
        refusal_reason=None if matched is not None else "generated_coordination_path_escape",
    )


def write_generated_coordination_output(
    path: str | Path,
    content: Mapping[str, object] | object | str,
    *,
    workspace_root: str | Path = ".",
) -> CoordinationPathPolicyResult:
    """Write generated coordination output only after path-policy acceptance."""

    policy = validate_generated_coordination_output_path(path, workspace_root=workspace_root)
